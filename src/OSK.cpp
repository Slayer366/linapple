...
static void MoveVertical(int direction)
{
  ...
  int currentCenter =
      g_keys[g_selected].x + g_keys[g_selected].w / 2;
...
bool OSK_HandleEvent(SDL_Event *event)
{
  ...
  switch (event->type)
  {
    case SDL_KEYDOWN:
    {
      ...
      /* Physical Escape closes the OSK.
       * This is deliberately handled here rather than activating
       * the selected virtual Escape key.
       */
      if (sym == SDLK_ESCAPE)
      {
        g_bOSKVisible = false;
...
void OSK_Draw(SDL_Surface *surface)
{
  ...
  for (int i = 0; i < g_keyCount; i++)
  {
    ...
    SDL_BlitSurface(g_OSKAlphaSheet,
                &src,
                surface,
                &dst);
